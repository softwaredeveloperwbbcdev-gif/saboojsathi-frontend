...
const DistrictViewPending = () => {
  ...
  const [listOfBlocks, setBlockList] = useState([]);
  ...
  // Fetch initial block list for the dropdown
  const onLoadHandler = async () => {
    ...
    try {
      ...
      if (!response.error && response.data) {
        ...
        toast.error("Failed to fetch block list");
      }
    } catch (err) {
  ...
  // Submit handler for fetching school table data
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      const response = await callApi("POST", `getDistrictVerifyList`, data);
      if (response.error) {
      ...
      <section className="p-4 md:p-10 bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors duration-300">
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-bold text-sm uppercase tracking-wider mb-2">
              ...
              District Overview • Phase {phaseDetails.phaseName}
            </div>
            <h1 className="text-2xl md:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              Block-wise <span className="text-indigo-600">Pending</span> Status
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">
              Monitoring application flow from blocks to district approval
              level.
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 mb-8">
          <div className="flex items-center gap-2 mb-6 text-slate-400 uppercase text-xs font-bold tracking-widest">
            <HiOutlineFunnel className="text-lg text-indigo-500" />
            Filter by Block Jurisdiction
          </div>
          <form className="max-w-md">
            <div className="relative group">
              <HiOutlineBuildingLibrary className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors w-5 h-5" />
              <select
                {...register("blockId")}
                onChange={(e) => handleOnChange("blockId", e.target.value)}
                className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-slate-900 dark:text-slate-100 focus:ring-4 focus:ring-indigo-500/10 transition-all appearance-none cursor-pointer font-medium"
              >
                <option value={btoa("all")}>All Institutions</option>
                <option value={btoa("pending")}>
                  Pending Institutions Only
                </option>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center justify-center text-slate-400">
                        ...
                        <p className="text-sm">
                          Try selecting a specific block
                        </p>
                      </div>
                    </td>
                  </tr>
...
export default DistrictViewPending;
