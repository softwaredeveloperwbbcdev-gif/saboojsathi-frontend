...
import { profileEntryStatusDist2023 } from "../../../Services/ApiCall";
import { useParams } from "react-router-dom";

const ProfileEntryStatusReportDist2023 = ({ phaseName = "X" }) => {
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState([]);

  const { year, phase } = useParams();

  useEffect(() => {
    // alert(year + phase);
    const fetchData = async () => {
      setLoading(true);
      const response = await profileEntryStatusDist2023(
        `profile-entry-status-report-district/${year}/${phase}`
      ); // API call
                  ...
                  {data.length === 0 ? (
                    ...
                    data.map((value, index) => (
                      <tr key={index} className="hover:bg-gray-100">
                        ...
                        <td className="text-center p-2 border border-gray-400">
                          <Link
                            to={`/ProfileEntryStatusReportBlock2023/${year}/${value.district_id_pk}`}
                          >
                            {value.district_name}
                          </Link>
                        </td>
...
export default ProfileEntryStatusReportDist2023;
