...
const TaggingDetailsReportOldBlock = () => {
  ...
  const fetchData = async () => {
    //////////////////////////////////////////////////////////////// school_tagging_report_block
    try {
      setLoading(true);
      const response = await callApi(
        "GET",
        `school_tagging_report_blockwise/${phaseId}/${id}`
      ); // API call
      if (response.error) {
      ...
      <AdminAuthenticatedLayout>
        <section className="p-4 md:p-8 lg:p-12 bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors duration-300">
          <h1 className="text-2xl md:text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-8 tracking-tight">
            Block Wise School Tagged Report {phaseDetails.phaseName}
            ...
            <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400 border-separate border-spacing-0">
              ...
              <tbody>
                {data.length === 0 ? (
                  ...
                  data.map((value, index) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <Link
                          to={`/TaggingDetailsReportOldSchool/${phaseId}/${btoa(
                            value.block_id_pk
                          )}`}
                        >
                          {value.block_name}
                ...
                <tr>
                  ...
                  <td className="p-4">{total_no_of_schools_tagged}</td>

                  <td className="p-4">{total_tagged_boys}</td>
                  <td className="p-4">{total_tagged_girls}</td>
                  <td className="p-4">{total_tagged}</td>

                  <td className="p-4">{total_eligible_boys}</td>
                  <td className="p-4">{total_eligible_girls}</td>
                  <td className="p-4">{total_eligible}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
export default TaggingDetailsReportOldBlock;
