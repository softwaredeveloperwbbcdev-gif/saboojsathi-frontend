...
function EligibleStudentReportBlock() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (response.error) {
        ...
        const filename = `Block_Wise_Eligible_Students_Phase_${phase}_${year}_${timestamp}.xlsx`;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await callApi(
        "GET",
        `repo-blockwise-eligible-student/${phaseId}/${id}`
      ); // API call
      if (response.error) {
      ...
      <AdminAuthenticatedLayout>
        <section className="p-4 md:p-8 lg:p-12 bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors duration-300">
          <h1 className="text-2xl md:text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-8 tracking-tight">
            Block Wise Eligible Student Report Phase {phaseDetails.phaseName}{" "}
            For Academic Year&nbsp;
            {phaseDetails.year}
          </h1>
                  ...
                  <>
                    {data.map((value, index) => (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <Link
                            to={`/EligibleStudentReportSchool/${phaseId}/${btoa(
                              value.block_id_pk
                            )}`}
                          >
                            {value.block_name}
