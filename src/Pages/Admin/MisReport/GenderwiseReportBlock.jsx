...
function GenderwiseReportBlock() {
  const { id } = useParams();
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await callApi("GET", `/genderwise_report_block/${id}`); // API call
      if (response.error) {
                ...
                {data.length === 0 ? (
                  ...
                  data.map((value, index) => (
                    ...
                    >
                      ...
                      <td className="p-4">{value.grand_total}</td>
                    </tr>
                  ))
                )}
                <tr className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
...
export default GenderwiseReportBlock;
