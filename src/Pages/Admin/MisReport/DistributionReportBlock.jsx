...
function DistributionReportBlock() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (response.error) {
        ...
        const filename = `Block_Wise_Distribution_Report_${phase}_${year}_${timestamp}.xlsx`;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await callApi(
        "GET",
        `blockwise_distribution_report/${id}/${phaseId}`
      ); // API call
      if (response.error) {
      ...
      <AdminAuthenticatedLayout>
        <section className="p-4 md:p-8 lg:p-12 bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors duration-300">
          <h1 className="text-2xl md:text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-8 tracking-tight">
            Block Wise Distribution Status Report (Phase{" "}
                  ...
                  <>
                    {data.map((value, index) => (
                      ...
                      >
                        <td className="p-4">
                          <Link
                            to={`/DistributionReportSchool/${btoa(
                              value.block_id
                            )}/${phaseId}`}
                          >
                            {value.block_name}
                    ...
                    <tr className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                      <td className="text-center font-semibold p-4">Total</td>
                      ...
                      <td className="font-semibold p-4">{updated_total}</td>
                    </tr>
                  </>
                )}
              </tbody>
