  ...
  const downloadPdf = async () => {
    ...
    try {
      const response = await callApi(
        "GET",
        `challan_payment_report/${phaseId}`
      );
      ...
      if (response.error) {
        ...
        link.href = blobUrl;
        link.download = filename || "download-challan-payment.pdf";
      ...
      <AdminAuthenticatedLayout>
        <section className="p-4 md:p-8 lg:p-12 bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors duration-300">
          <h1 className="text-2xl md:text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-8 tracking-tight">
            Challan Payment Report (Phase {phaseDetails.phaseName})
          </h1>
              ...
              <tbody>
                ...
                >
                  <td className="p-4 text-center">
                    {phaseDetails.phaseName}/{phaseDetails.year}
                  </td>
                  <td className="p-4 text-center">Challan Payment Report</td>
                  <td className="p-4 text-center">
                    <button
                      onClick={downloadPdf}
                      className="bg-blue-900 text-white p-2 rounded-md hover:bg-blue-700"
                    >
                      Download Challan Payment Report
                    </button>
                  </td>
                </tr>
              </tbody>
...
export default ChallanPayementReport;
