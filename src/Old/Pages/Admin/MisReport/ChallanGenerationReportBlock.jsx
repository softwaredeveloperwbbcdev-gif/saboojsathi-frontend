  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await callApi(
        "GET",
        `challan_generation_report_block/${phaseId}/${id}`
      ); // API call
      if (response.error) {
      ...
      <AdminAuthenticatedLayout>
        <section className="p-4 md:p-8 lg:p-12 bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors duration-300">
          ...
          <div className="w-full pb-2 flex justify-end">
            {/* Download Button */}
            {/* <button className="relative bg-sky-500 text-white px-4 py-2 rounded-md hover:bg-sky-600 focus:outline-none">
              Download
            </button> */}
          </div>
            ...
            <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400 border-separate border-spacing-0">
              ...
              <tbody>
                  ...
                  <>
                    {data.map((value, index) => (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <Link
                            to={`/ChallanGenerationReportSchool/${phaseId}/${btoa(
                              value.block_id_pk
                            )}`}
                          >
                            {value.block_name}
                ...
                <tr className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                  ...
                  <td className="font-semibold p-4">{boys_allocation_total}</td>
                  <td className="font-semibold p-4">
                  ...
                  <td className="font-semibold p-4">{boys_updated_total}</td>
                  <td className="font-semibold p-4">{girls_updated_total}</td>
                  <td className="font-semibold p-4">{total_updated_total}</td>
                </tr>
              </tbody>
            </table>
          </div>
