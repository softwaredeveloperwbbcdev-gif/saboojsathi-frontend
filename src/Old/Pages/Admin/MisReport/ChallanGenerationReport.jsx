...
function ChallanGenerationReport() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await callApi(
        "GET",
        `challan_generation_report/${phaseId}`
      ); // API call
      if (response.error) {
  ...
  const boys_allocation_total = data.reduce((acc, value) => {
    return acc + value.boys_allocation; // (Number(value.boys_allocation) || 0) //change the view or function data type
  }, 0);
  const girls_allocation_total = data.reduce((acc, value) => {
    return acc + value.girls_allocation;
  }, 0);
  const total_allocation_total = data.reduce((acc, value) => {
    return acc + value.total_allocation;
  }, 0);
            ...
            <table className="w-full text-left text-sm text-gray-500 dark:text-gray-400 border-separate border-spacing-0">
              <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                  ...
                  <th colSpan="3" className="p-4 text-center rounded-tr-lg">
                  ...
                  <>
                    {data.map((value, index) => (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <Link
                            to={`/ChallanGenerationReportBlock/${phaseId}/${btoa(
                        ...
                        <td className="p-4">
                          {value.boys_allocation ? value.boys_allocation : 0}
                        </td>
                        <td className="p-4">
                          {value.girls_allocation ? value.girls_allocation : 0}
                        </td>
                        <td className="p-4">
                          {value.total_allocation ? value.total_allocation : 0}
                        </td>
                    ...
                    <tr className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
                      ...
                      <td className="p-4">{boys_musterroll_generated_total}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4">{boys_allocation_total}</td>
                      <td className="p-4">{girls_allocation_total}</td>
                      <td className="p-4">{total_allocation_total}</td>

                      <td className="p-4">{boys_updated_total}</td>
                      <td className="p-4">{girls_updated_total}</td>
                      <td className="p-4">{total_updated_total}</td>
                    </tr>
                  </>
                )}
              </tbody>
